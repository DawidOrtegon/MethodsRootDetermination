...
{
    public double falseMethod(FunctionZadanie2 function, double xl, double xu, double error)
    {
        ...
        do
        {
            // The only thing that changes is the xr, the rest we leave the same.
            xr = xu - (function.functionValue(xu)*(xl-xu))/(function.functionValue(xl)-function.functionValue(xu));
            valuesXr.add(xr);
            ...
            if (iterator == 1)
            {
                ...
                // trueFunctionError = Math.abs((function.getTrueValue()-function.functionValue(xr))/function.getTrueValue())*100;
                trueFunctionError = Math.abs((function.getTrueValue()-(xr))/function.getTrueValue())*100;
            ...
            // Secure the advance of the iteration.
            if(iterator == 1)
            {
                iterator ++;
                continue;
