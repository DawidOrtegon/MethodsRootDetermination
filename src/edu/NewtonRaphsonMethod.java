...
{
    public double nRmethod(FunctionZadanie2 function, double xn, int maxIterations, double error)
    {
        ...
        xrApp[iterator-1] = xn - (function.functionValue(xn)/function.firstDerivative(xn));

        do
        {
            xrApp[iterator] = xrApp[iterator-1] - function.functionValue(xrApp[iterator-1])/function.firstDerivative(xrApp[iterator-1]);
            ...
            if(iterator > 1)
            {
                ...
                trueFunctionError = Math.abs((function.getTrueValue()-xrApp[iterator]/function.getTrueValue()))*100;
